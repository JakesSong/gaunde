...
모임 중간 지점 찾기. 수도권 전철 기준으로 **"제일 오래 걸리는 사람의 시간을 가장 줄이는 역"** 을 찾아준다.
결과 화면에서 기준을 **시간·요금·환승** 중에 골라 볼 수 있다 (기본은 시간).
...
- **급행을 쓰지 않는다.** 각역정차 기준이라 급행이 있는 구간은 실제보다 느리게 나온다.
  넣으려면 (1) 노선별 급행 정차역 목록, (2) 급행/완행 배차와 완행→급행 갈아타기 대기,
  (3) 같은 역쌍의 두 계통 중 빠른 쪽을 고르는 규칙이 필요하다. 셋 다 OSM 관계에는 없다.
  **환승 기준**에서 특히 걸린다 — 급행 한 번이 완행 두 번보다 나은 판을 지금은 못 본다.
